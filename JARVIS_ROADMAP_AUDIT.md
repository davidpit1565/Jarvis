...
| 49 | Local Model Adapter | ALREADY_EXISTS_AND_GOOD (REQUIRES_USER_SETUP for reachability) | `src/core/brain/OllamaBrain.ts` (new); registered in `AIProviderRegistry` as a fourth `"free"`-tier provider when `OLLAMA_MODEL` is set | Implemented this pass: an `OllamaBrain implements Brain` talking to a user-run [Ollama](https://ollama.com) server's OpenAI-compatible `chat/completions` endpoint, following `GroqBrain`/`OpenRouterBrain`'s exact message/tool-call-translation shape (reused, not reinvented — same `toOpenAIMessages`/`toOpenAITools`/`fromOpenAIResponse` logic). No API key is ever required (an optional `OLLAMA_API_KEY` bearer token exists only for a reverse-proxied setup); no `:free`-suffix gate is needed the way `OpenRouterBrain` needs one, since a locally-run model has no paid tier at all — `CostTracker.estimateCostUsd` is unconditionally `$0` for it. Uses `fetchWithRetry` with a 30s default timeout (3x `fetchWithRetry`'s own 10s default) since local inference on modest hardware is genuinely slower than a hosted API. **REQUIRES_USER_SETUP caveat, called out explicitly in the class doc comment and README**: this only works out of the box when JARVIS's backend and Ollama run on the same machine/network — `OLLAMA_BASE_URL` defaulting to `http://localhost:11434` resolves to wherever JARVIS's own process runs, not the user's Mac, if JARVIS is deployed to a cloud host like Fly.io. Bridging that (Tailscale/ngrok) is the user's own responsibility, not something this pass implements. `AIRouter`'s existing circuit breaker was verified (new test in `tests/brain/AIRouter.test.ts`, "a genuinely unreachable Ollama server...") to open and route around a real, unreachable `OllamaBrain` without blocking other configured providers — the most likely real-world failure mode for this specific provider. |
