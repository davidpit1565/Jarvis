...
| 46 | Dynamic Model Catalog | MISSING | none | Support a configurable list of providers/models instead of the `ProviderName = "anthropic" \| "groq"` closed union. | Confirmed: only two hardcoded providers anywhere in `AIProviderRegistry.ts`/`AIRouter.ts`. |
| 47 | Task-Type Routing | MISSING | none | Route different task types (vision, code, chat) to different models/providers. | `AIRouter` routes purely by cost tier + explicit pin, never by what the task actually needs. |
...
| 50 | OpenRouter Free Provider | MISSING | none | Add an `OpenRouterBrain` implementing the `Brain` interface. | Not referenced anywhere in the codebase. |
| 51 | OpenRouter Capability Filtering | MISSING | none | Depends on #50. | Blocked on OpenRouter integration existing first. |
...
| 58 | Prompt Cache | MISSING | none — confirmed zero `cache_control` occurrences repo-wide | Add Anthropic prompt caching (`cache_control: {type: "ephemeral"}`) on the system prompt/tools array. | This is `JARVIS_AUDIT.md`'s own top gap item #9 — `ClaudeBrain.ts` reads back `cache_creation_input_tokens`/`cache_read_input_tokens` from responses but never sets a cache breakpoint on requests, so caching never actually activates. Straightforward, high-value fix. |
| 59 | Result Cache | MISSING | none | Cache tool-call results for repeat/idempotent read calls (e.g. same weather query within N minutes). | No caching layer found around any tool's `execute()`. |
| 60 | Semantic Cache | MISSING | none | Add embedding-based caching for semantically-similar prompts. | Requires embeddings infrastructure that doesn't exist anywhere in the repo (`MemoryStore` search is plain SQL `LIKE`, confirmed in `JARVIS_AUDIT.md` Phase 7). |
...
| 145 | Search Cache | MISSING | none | Same underlying gap as #59 (Result Cache) applied to search specifically. | No caching anywhere in the search tools. |
...
| # | Feature | Status | Existing Implementation (files) | Required Work | Notes |
|---|---|---|---|---|---|
| 186 | JARVIS Command Center | MISSING | none — the hologram UI is a themed chat/activity overlay, not a command-center dashboard | Build a dedicated admin/command-center view (distinct from the hologram itself). | The existing UI is intentionally hologram-themed/minimal, not a data-dense operator dashboard. |
| 187 | Live Agent Monitor | PARTIALLY_EXISTS | Activity feed + observer WS stream (admin-only) | Extend into a dedicated live agent-execution monitor (current tool call, current iteration of the tool loop). | Activity feed shows completed events; no live "here's what the agent is doing right now, step N of the loop" view. |
| 188 | Provider Monitor | MISSING | none — blocked on #6/#43/#44 (no provider health/latency/success-rate data collected at all) | Build once underlying provider metrics exist. | No data to monitor yet. |
| 189 | Memory Explorer | MISSING | Same gap as #110 | Build a searchable/browsable memory UI. | No UI exists; `MemoryStore` itself supports the needed search primitive already. |
| 190 | Automation Explorer | MISSING | `LIST_AUTOMATION_RULES` tool exists (chat-invoked) but no dedicated UI view | Build a UI panel listing/editing automation rules directly (not via chat). | Backend CRUD is complete and tested; only reachable through conversational tool calls today, no UI. |
| 191 | Audit Explorer | MISSING | `ToolAuditLog` data exists and is queryable, but no dedicated UI browses it | Build a UI to browse/filter the audit log. | `/audit-log` admin endpoint exists per `JARVIS_AUDIT.md` Phase 6 (gated by admin token "if configured"); no rich browsing UI confirmed on top of it. |
| 192 | Cost Analytics | MISSING | Same gap as #109/#55/#56/#57 | Build once a UI + suggestion layer exists on top of `CostTracker`/`TokenUsageStore`. | Real underlying data, zero analytics UI. |
| 193 | Performance Analytics | MISSING | Blocked on #43/#44/#179 (no latency/success-rate data collected) | Build once underlying metrics exist. | No data to analyze yet. |
| 194 | "Why did you do this?" explainability | MISSING | none | Add a reasoning-trace surface tied to `ToolAuditLog`/`EventBus` events (Claude's own tool-call reasoning isn't currently persisted/exposed distinctly). | No explainability feature found; `brain.response` events capture `text`/`toolCalls` but not a structured "reason" field distinct from the reply text. |
| 195 | "What are you doing?" status | PARTIALLY_EXISTS | Activity feed shows recent/completed actions | Add a live "currently doing X" status distinct from the historical feed (ties to #34/#36/#187). | Retrospective, not live-status. |
| 196 | Stop button | MISSING | Same gap as #20 (Agent Cancellation) | Build UI control once a cancellation mechanism exists at the backend (#20). | No backend cancellation to wire a button to yet. |
| 197 | Undo Where Safe | ALREADY_EXISTS_NEEDS_UPGRADE | `src/core/undo/UndoStore.ts`, `UNDO_LAST_ACTION` tool (`src/tools/undo/UndoLastActionTool.ts`), used by `DELETE_REMINDER` | Extend undo coverage beyond reminders to other reversible actions (e.g. `DELETE_AUTOMATION_RULE`, `SAVE_MEMORY` overwrites). | Real, tested (`tests/core/UndoStore.test.ts`, `tests/tools/UndoLastActionTool.test.ts`); scope today is narrow (confirmed reminder deletion is the primary wired case). |
| 198 | Action Preview | PARTIALLY_EXISTS | CONFIRM-level tools show *some* confirmation prompt before executing | Add a rich structured preview (not just a generic "confirm this tool call?") for the highest-stakes actions (email send, file write, trash empty). | Same evidence/gap as #139 — confirmation gating is real, preview *richness* not independently confirmed. |
...
| 200 | Device Approval Web UI | ALREADY_EXISTS_AND_GOOD | `PairingService`, `DeviceRegistry`, `/pairing-approve`/`/pairing-revoke` HTTP endpoints, tested (`tests/integration/pairingApprovalHttp.test.ts`, `pairingRevokeHttp.test.ts`) | Confirm the hologram UI actually exposes an approve/revoke button (endpoints are confirmed; the specific UI affordance wasn't independently re-traced pixel-by-pixel this pass). | Backend is real, tested, and admin-token-gated. |
| 201 | Real User Identity | NOT_WORTH_IMPLEMENTING | `userId` is a simple string/constant throughout (single-user personal assistant design) | N/A unless multi-user support is actually wanted. | This is a single-owner (David) personal assistant by design — full multi-user identity (OAuth users, roles) is unjustified complexity for a one-person system unless David explicitly wants to add other users. |
...
| 203 | Session Management | PARTIALLY_EXISTS | `src/auth/SessionStore.ts` (cookie sessions with expiry) | Add a multi-session view (list all active sessions), not just single-session expiry logic. | Expiry/validity checking is real and tested; no "list/manage all my sessions" UI or API. |
| 204 | Session Revocation | PARTIALLY_EXISTS | `SessionStore` deletes on expiry check; `PairingService.revoke()` exists for device credentials | Add explicit user-initiated session revocation (today expiry is time-based/automatic, not something the user can trigger on demand from a UI for a *dashboard login* session specifically). | Device-credential revocation is real and explicit (`PairingService.revoke`); dashboard-login session revocation is only implicit (via expiry), not user-triggered. |
| 205 | Permission Management UI | MISSING | `PermissionService` grant/check logic exists (backend), but no UI to view/revoke standing grants was found | Build a UI to list and revoke `(userId, toolId, deviceId)` grants. | Backend model supports this; no UI surfaces it. |
| 206 | Security Events UI | MISSING | Confirmation attempts, permission denials, lockdown activation are all logged (`ActivityLog`, `ToolAuditLog`) but not surfaced as a distinct "security events" view | Build a filtered security-events UI on top of existing logs. | Data exists; no dedicated security-focused view. |
| 207 | Emergency Lockdown UI | ALREADY_EXISTS_AND_GOOD | `/emergency-lockdown` endpoint + `LockdownService`, tested (`tests/integration/emergencyLockdownHttp.test.ts`) | Confirm a corresponding button exists in the hologram UI (endpoint confirmed; exact UI affordance not pixel-traced this pass). | Backend is real, tested, and does exactly what it claims (blocks everything above READ). |
| 208 | Config Center | MISSING | `src/config/index.ts` (`loadConfig()`) reads ~35 env vars; `.env.example` documents them all | Build a UI to view (and where safe, edit) configuration — today it's entirely env-var/redeploy-based, no runtime config UI. | Configuration is real and well-documented in code/`.env.example`, but is a deploy-time concern only, not a runtime UI concern. |
| 209 | Safe Mode | MISSING | `LockdownService`'s emergency lockdown is close but distinct (blocks all non-READ actions; not a reduced-feature-set "safe mode" for e.g. degraded-provider situations) | Build a genuine "safe mode" (e.g. auto-triggered on repeated tool failures, distinct from the manually-triggered lockdown). | Lockdown exists and is well-built but is a different concept (a deliberate kill switch, not an automatic degraded-operation mode). |
| 210 | Recovery Mode | MISSING | none | Build an explicit recovery-mode flow (e.g. guided restore from backup, per #12/#164/#165 gaps). | No such mode exists; ties directly to the missing backup-restore path noted in `JARVIS_AUDIT.md` gap #12. |
...
| Status | Count |
|---|---|
| ALREADY_EXISTS_AND_GOOD | 26 |
| ALREADY_EXISTS_NEEDS_UPGRADE | 13 |
| PARTIALLY_EXISTS | 47 |
| MISSING | 107 |
| REQUIRES_REAL_MAC_VALIDATION | 16 |
| REQUIRES_USER_SETUP | 0 (see note) |
| NOT_WORTH_IMPLEMENTING | 1 |
| BLOCKED | 0 (see note) |
| **Total** | **210** |

(Counts independently verified by scripting a regex count over the numbered table rows themselves, not hand-tallied.)
