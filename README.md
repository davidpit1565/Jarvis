...
now checks `ReminderStore.getDueUnnotified()` and pushes a real Telegram
notification — "⏰ Reminder: ...", so it actually reaches the user instead
of waiting to be asked. A `notified_at` column (persisted, so this
survives a restart — no in-memory-only state to lose) stops the same
reminder from notifying twice; editing a reminder to a new due date
(`UPDATE_REMINDER`) clears it, so a re-dated reminder gets a fresh
notification at its new time rather than staying silenced by one already
